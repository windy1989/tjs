...
class ReportProjectController extends Controller {
	...
	public function datatable(Request $request) 
    {
        ...
        if($query_data <> FALSE) {
            ...
            foreach($query_data as $val) {
                if($val->progress == 100) {
					$action = '<a href="' . url('admin/report/project/progress/' . $val->id) . '" class="btn bg-success btn-sm" data-popup="tooltip" title="Detail"><i class="icon-search4"></i></a>';
					...
					$action = '<a href="' . url('admin/report/project/progress/' . $val->id) . '" class="btn bg-warning btn-sm" data-popup="tooltip" title="Progress"><i class="icon-zoomin3"></i></a>';
	...
	public function progress(Request $request, $id)
    {
        ...
        if($request->has('_token') && session()->token() == $request->_token) {
            ...
            if($validation->fails()) {
                return redirect('admin/report/project/progress/' . $id . '?' . $step . '=1#' . $step)
			...
			$data = [
				...
				'content'		=> 'admin.report.project_progress'
			];
	...
	public function print($param, $id)
    {
        if($param == 'quotation_order'){
			...
			$pdf = PDF::loadView('admin.pdf.project.' . $param, [
					'project' => $project,
					'brand'   => Brand::whereIn('code', ['TR', 'FI', 'SM', 'BT', 'BV', 'GTI'])->get()
				],
				[],
				[ 
				  'format' => 'A4-P',
