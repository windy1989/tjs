...
class CogsController extends Controller {
    ...
    public function getCompleteData(Request $request)
    {
        ...
        return response()->json([
            ...
            'container'              => $c,
            'product_price'          => $symbol . (!is_nan($pp) && !is_infinite($pp) ? number_format(round($pp), 0, ',', '.') : 0),
            'buying_unit'            => $data->type->buyUnit->code,
            'selling_unit'           => $data->type->sellingUnit->code,
            'conversion_unit'        => $symbol . (!is_nan($cu) && !is_infinite($cu) ? number_format(round($cu), 0, ',', '.') : 0),
            'rate_unit'              => $symbol . (!is_nan($ru) && !is_infinite($ru) ? number_format(round($ru), 0, ',', '.') : 0),
            'local_price_idr'        => $symbol . (!is_nan($lpi) && !is_infinite($lpi) ? number_format(round($lpi), 0, ',', '.') : 0),
            'total_sqm_load'         => (!is_nan($tsl) && !is_infinite($tsl) ? number_format(round($tsl), 0, ',', '.') : 0) . ' <sub>/ CONTAINER</sub>',
            'agent_fee_usd_sqm'      => (!is_nan($afus) && !is_infinite($afus) ? number_format(round($afus), 3, ',', '.') : 0) . ' <sub>/ SQM</sub>',
            'agent_fee_idr'          => $symbol . (!is_nan($afi) && !is_infinite($afi) ? number_format(round($afi), 0, ',', '.') : 0),
            'freight_cost_usd'       => $symbol . (!is_nan($fcu) && !is_infinite($fcu) ? number_format(round($fcu), 0, ',', '.') : 0) . ' <sub>/ CONTAINER</sub>',
            'cbm_container'          => (!is_nan($cc) && !is_infinite($cc) ? number_format(round($cc), 0, ',', '.') : 0) . ' <sub>/ CONTAINER</sub>',
            'kg_dos'                 => number_format(round($wg), 0, ',', '.') . ' Kg',
            ...
            'landed_cost_container'  => $symbol . (!is_nan($lcc) && !is_infinite($lcc) ? number_format(round($lcc), 0, ',', '.') : 0) . ' <sub>/ CONTAINER</sub>',
            'total_landed_cost'      => $symbol . (!is_nan($tlc) && !is_infinite($tlc) ? number_format(round($tlc), 0, ',', '.') : 0) . ' <sub>/ SQM</sub>',
            'rate_of_usd'            => $symbol . (!is_nan($ru) && !is_infinite($ru) ? number_format(round($ru), 0, ',', '.') : 0),
            ...
            'safe_guard'             => $symbol . (!is_nan($sg) && !is_infinite($sg) ? number_format(round($sg), 0, ',', '.') : 0) . ' <sub>/ SQM</sub>',
            'cogs_idr'               => $symbol . (!is_nan($ci) && !is_infinite($ci) ? number_format(round($ci), 0, ',', '.') : 0) . ' <sub>/ SQM</sub>',
            'cogs_pta_idr'           => $symbol . (!is_nan($cpi) && !is_infinite($cpi) ? number_format(round($cpi), 0, ',', '.') : 0) . ' <sub>/ SQM</sub>',
            'cogs_smb_idr'           => $symbol . (!is_nan($csi) && !is_infinite($csi) ? number_format(round($csi), 0, ',', '.') : 0) . ' <sub>/ SQM</sub>'
        ]);
    }
