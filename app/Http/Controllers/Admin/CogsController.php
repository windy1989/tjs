...
class CogsController extends Controller {
    ...
    public function getCompleteData(Request $request)
    {
        ...
        return response()->json([
            ...
            'rate_unit'              => $symbol . (!is_nan($ru) && !is_infinite($ru) ? number_format($ru, 2, ',', '.') : 0),
            ...
            'cbm_container'          => (!is_nan($cc) && !is_infinite($cc) ? number_format(round($cc), 2, ',', '.') : 0) . ' <sub>/ container</sub>',
            ...
            'tonnage_of_container'   => (!is_nan($toc) && !is_infinite($toc) ? number_format(round($toc), 2, ',', '.') : 0) . '%',
            'sqm_dos'                => (!is_nan($sd) && !is_infinite($sd) ? number_format(round($sd), 2, ',', '.') : 0) . ' <sub>/ DOS</sub>',
            'freight_cost'           => $symbol . (!is_nan($fc) && !is_infinite($fc) ? number_format(round($fc), 2, ',', '.') : 0),
            'landed_cost_container'  => $symbol . (!is_nan($lcc) && !is_infinite($lcc) ? number_format($lcc, 2, ',', '.') : 0) . ' <sub>/ container</sub>',
            'total_landed_cost'      => $symbol . (!is_nan($tlc) && !is_infinite($tlc) ? number_format(round($tlc), 2, ',', '.') : 0) . ' <sub>/ SQM</sub>',
            'rate_of_usd'            => '$' . (!is_nan($ru) && !is_infinite($ru) ? number_format($ru, 2, ',', '.') : 0),
            'ls_cost_sqm'            => $symbol . (!is_nan($lcs) && !is_infinite($lcs) ? number_format(round($lcs), 2, ',', '.') : 0) . ' <sub>/ SQM</sub>',
            'import_duty'            => $symbol . (!is_nan($id) && !is_infinite($id) ? number_format(round($id), 2, ',', '.') : 0) . ' <sub>/ SQM</sub>',
            'value_tax'              => $symbol . (!is_nan($vt) && !is_infinite($vt) ? number_format(round($vt), 2, ',', '.') : 0) . ' <sub>/ SQM</sub>',
            'income_tax'             => $symbol . (!is_nan($it) && !is_infinite($it) ? number_format(round($it), 2, ',', '.') : 0) . ' <sub>/ SQM</sub>',
            'total_import_tax'       => $symbol . (!is_nan($tit) && !is_infinite($tit) ? number_format(round($tit), 2, ',', '.') : 0) . ' <sub>/ SQM</sub>',
            'safe_guard'             => $symbol . (!is_nan($sg) && !is_infinite($sg) ? number_format($sg, 2, ',', '.') : 0) . ' <sub>/ SQM</sub>',
    ...
    public function show(Request $request)
    {
        ...
        return response()->json([
            ...
            'ls_cost_document'       => $symbol . number_format($data->ls_cost_document, 2, ',', '.'),
            'number_container'       => $data->number_container,
            'sni_cost'               => $symbol . number_format($data->sni_cost, 2, ',', '.'),
            ...
            'rate_unit'              => $symbol . number_format($formula->rate_unit, 2, ',', '.'),
            ...
            'cbm_container'          => number_format($formula->cbm_container, 2, ',', '.') . ' <sub>/ container</sub>',
            ...
            'tonnage_of_container'   => number_format($formula->tonnage_of_container, 2, ',', '.') . '%',
            'sqm_dos'                => number_format($formula->sqm_dos, 2, ',', '.') . ' <sub>/ DOS</sub>',
            'freight_cost'           => $symbol . number_format($formula->freight_cost, 2, ',', '.'),
            'landed_cost_container'  => $symbol . number_format($formula->landed_cost_container, 2, ',', '.') . ' <sub>/ CONTAINER</sub>',
            'total_landed_cost'      => $symbol . number_format($formula->total_landed_cost, 2, ',', '.') . ' <sub>/ SQM</sub>',
            'rate_of_usd'            => '$' . number_format($formula->rate_of_usd, 2, ',', '.'),
            'ls_cost_sqm'            => $symbol . number_format($formula->ls_cost_sqm, 2, ',', '.') . ' <sub>/ SQM</sub>',
            'import_duty'            => $symbol . number_format($formula->import_duty, 2, ',', '.') . ' <sub>/ SQM</sub>',
            'value_tax'              => $symbol . number_format($formula->value_tax, 2, ',', '.') . ' <sub>/ SQM</sub>',
            'income_tax'             => $symbol . number_format($formula->income_tax, 2, ',', '.') . ' <sub>/ SQM</sub>',
            'total_import_tax'       => $symbol . number_format($formula->total_import_tax, 2, ',', '.') . ' <sub>/ SQM</sub>',
            'safe_guard'             => $symbol . number_format($formula->safe_guard, 2, ',', '.') . ' <sub>/ SQM</sub>',
