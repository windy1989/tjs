					...
					<div class="card">
						...
						<div class="card-body">
							<div class="list-feed">
								...
								@endforeach
								@foreach($finish as $fn)
								<div class="list-feed-item border-warning-400">
									<div class="text-muted font-size-sm mb-1">{{ $fn->date_receive }}</div>
									Received by mr/mrs <b>{{ $fn->person }}</b> with Warehouse Code {{ $fn->code }} proof : {!! $fn->image ? '<a href="' . $fn->attachment() . '" data-lightbox="' . $fn->code . '" data-title="' . $fn->code . '"><img src="' . $fn->attachment() . '" style="max-width:70px;" class="img-fluid img-thumbnail mb-2"></a>' : '<span class="badge badge-secondary">None</span>' !!}
								</div>
								@endforeach
							</div>
						</div>
